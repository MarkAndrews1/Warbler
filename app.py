...
@app.route('/users/profile/<int:user_id>/likes')
def show_liked_msgs(user_id):
    ...
    return render_template('users/liked-msgs.html', user=user, likes=user.likes)
...
def homepage():
    ...
    if g.user:
        if g.user.following:
            """Checks if user is following anyone"""
            ...
            """If user isn't following anyone get the 100 most recent messages for anyone"""
            messages = (Message
                    .query
                    .order_by(Message.timestamp.desc())
                    .limit(100)
